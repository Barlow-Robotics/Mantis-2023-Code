...
public class PPCustomCommand extends CommandBase {
    ...
    private static BiConsumer<Translation2d, Rotation2d> logError = PPCustomCommand::defaultLogError;

    private static ArrayList<Translation2d> cargoLocations;
    private Vision visionSub;
    private PIDController cargoPID;

    /**
     * Constructs a new PPRamseteCommand that, when executed, will follow the
     * provided trajectory. PID
     ...
     * @param poseSupplier     A function that supplies the robot pose - use one of
     *                         the odometry classes
     *                         to provide this.
     * @param controller       The RAMSETE controller used to follow the trajectory.
     * @param feedforward      The feedforward to use for the drive.
     * @param kinematics       The kinematics for the robot drivetrain.
     * @param speedsSupplier   A function that supplies the speeds of the left and
     *                         right sides of the
     *                         robot drive.
     ...
     *
     * @param trajectory            The trajectory to follow.
     * @param poseSupplier          A function that supplies the robot pose - use
     *                              one of the odometry classes
     *                              to provide this.
     * @param controller            The RAMSETE follower used to follow the
     *                              trajectory.
     * @param kinematics            The kinematics for the robot drivetrain.
    ...
    public void initialize() {
        ...
        this.prevSpeeds = this.kinematics.toWheelSpeeds(
                new ChassisSpeeds(
    ...
    public void execute() {
        ...
        PathPlannerTrajectory.PathPlannerState desiredState = (PathPlannerTrajectory.PathPlannerState) transformedTrajectory
                .sample(currentTime);
        ...
        DifferentialDriveWheelSpeeds targetWheelSpeeds = this.kinematics.toWheelSpeeds(targetChassisSpeeds);
        ...
        if (this.usePID) {
            double leftFeedforward = this.feedforward.calculate(
                    leftSpeedSetpoint, (leftSpeedSetpoint - this.prevSpeeds.leftMetersPerSecond) / dt);
            double rightFeedforward = this.feedforward.calculate(
     ...
     * configuration of pushing values to SmartDashboard
     *
    ...
    public static void setLoggingCallbacks(
        ...
        PPCustomCommand.logActiveTrajectory = logActiveTrajectory;
        PPCustomCommand.logTargetPose = logTargetPose;
        PPCustomCommand.logSetpoint = logSetpoint;
        PPCustomCommand.logError = logError;
    }
}